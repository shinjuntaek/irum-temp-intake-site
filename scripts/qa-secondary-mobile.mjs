    ...
    window.fetch = async (input, init = {}) => {
      if (String(input).includes('temporary-secondary-profile')) {
        ...
        if (body.action === 'secondary-submit') {
          ...
          return new Response(JSON.stringify({ ok:true, status:'submitted', build_id:'secondary-submit-cas-current-payload-20260826-2', submitted_at:new Date().toISOString() }), { status:200, headers:{'Content-Type':'application/json'} });
        }
      }
