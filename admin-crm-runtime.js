(() => {
  ...
  window.installIrumCrmRuntime = () => {
    ...
    const controlMarkup = (field, value) => {
      ...
      // 선택지를 새 기준으로 바꿔도 기존 payload의 값을 지우거나 빈 선택으로 보이지 않게 한다.
      const options = current !== "" && !configured.some((option) => optionValue(option) === current) ? [...configured, current] : configured;
      if (field.control === "chips" && options.length >= 3) return `<select data-edit-value><option value="">선택</option>${options.map((option) => `<option value="${esc(optionValue(option))}" ${optionValue(option) === current ? "selected" : ""}>${esc(valueForDisplay(option, field.key))}</option>`).join("")}</select>`;
      if (field.control === "chips") return `<div class="crm-option-row" data-option-group data-value="${esc(current)}" data-value-type="${typeof options?.[0] === "boolean" ? "boolean" : "string"}">${options.map((option) => `<button type="button" class="crm-option ${optionValue(option) === current ? "selected" : ""}" data-option-value="${esc(optionValue(option))}"><span>${esc(valueForDisplay(option, field.key))}</span><i class="crm-chip-mark"></i></button>`).join("")}</div>`;
      if (field.control === "select") return `<select data-edit-value><option value="">선택</option>${options.map((option) => `<option value="${esc(optionValue(option))}" ${optionValue(option) === current ? "selected" : ""}>${esc(valueForDisplay(option, field.key))}</option>`).join("")}</select>`;
