(() => {
  ...
  window.installIrumCrmRuntime = () => {
    ...
    const fieldCard = (item, field) => {
      ...
      return `<article class="crm-field ${filled(current) ? "" : "missing"}" data-direct-field="${esc(field.id)}"><div class="crm-field-head"><label>${esc(field.label)} <small class="crm-required">${field.required ? "필수" : "선택"}</small></label><span class="crm-source">${esc(field.sourceLabel)}</span></div><div class="crm-original">신청 내용 · ${esc(valueForDisplay(source.original, source.key))}</div><div class="crm-current">입력 내용 · ${esc(valueForDisplay(current, source.key))}</div>${field.locked ? '<div class="crm-readonly">기본 정보는 수정할 수 없습니다.</div>' : `<div class="crm-control" data-direct-autosave>${controlMarkup(field, current)}</div>`}</article>`;
    };
    ...
    const historyRail = (item) => {
      ...
      return `<aside class="crm-change-rail" aria-label="정보 변경 이력"><header><h3>정보 변경 이력</h3><p>신청 내용과 이후 확인 내용을 비교합니다.</p></header><div class="crm-history-filters">${filterOptions.map(([value, label]) => `<button type="button" class="${historyFilter === value ? "active" : ""}" data-history-filter="${value}">${label}</button>`).join("")}</div><ul class="crm-history-list">${rows.map((row) => `<li><b>${esc(row.title)}</b><small>${esc(row.source)} · ${esc(row.reason)}${row.reasonNote ? ` · ${esc(row.reasonNote)}` : ""}</small>${row.requested ? '<span class="customer-request-badge">고객 요청 변경</span>' : ""}<div class="crm-history-values"><div><span>신청 내용</span><strong>${esc(valueForDisplay(row.original))}</strong></div><div><span>이전 확인 내용</span><strong>${esc(valueForDisplay(row.previous))}</strong></div><div><span>수정 내용</span><strong>${esc(valueForDisplay(row.next))}</strong></div></div><small>${esc(row.who || "운영자")} · ${esc(date(row.at))}</small></li>`).join("") || '<li><small>선택한 조건의 변경 이력이 없습니다.</small></li>'}</ul></aside>`;
    };
